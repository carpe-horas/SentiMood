...
export const summarizeAndSaveDiary = async (chatroomId) => {
    try {
        const response = await api.post('/diary/summary/save', { chatroom_id: chatroomId });
        return response.data;
    } catch (error) {
...
export const getDiaryList = async (date) => {
    try {
        const response = await api.get('/diary/list', { params: { date } });
...
export const updateDiary = async (diaryId, content, emotion) => {
    try {
        const response = await api.put(`/diary/update/${diaryId}`, { content, emotion });
        return response.data;
    } catch (error) {
