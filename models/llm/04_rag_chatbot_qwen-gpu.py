...
# 모델 경로 설정 (현재 `data/models/Qwen2.5-3B-Instruct`에 있음)
MODEL_PATH = os.path.normpath(os.path.join(BASE_DIR, "data", "models", "Qwen2.5-3B-Instruct"))
...
print(f"로컬 Qwen2.5-3B-Instruct 모델 로드 중... ({MODEL_PATH})")
...
try:
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        # cpu 사용시 위 두개 주석, 아래 두 개 주석 해제
        # torch_dtype=torch.float32,  # float32 사용 (float16 제거)
        # device_map="cpu",  # 무조건 CPU에서 실행
...
# cpu 사용 시 아래 코드 주석
# FAISS 벡터 DB를 GPU로 변환 (GPU 사용 가능할 경우)
if torch.cuda.is_available():
    print("GPU 사용 가능, FAISS 인덱스를 GPU로 변환 중...")
    gpu_res = faiss.StandardGpuResources()
    index = vectorstore.index
    index = faiss.index_cpu_to_gpu(gpu_res, 0, index)
    vectorstore.index = index
    print("FAISS 인덱스 GPU 변환 완료.")
else:
    print("GPU 사용 불가, CPU에서 FAISS 인덱스를 사용합니다.")
